 ...
 */
sealed class UnitWiseEvent(val name: String, val params: Map<String, Any> = emptyMap()) {
    
    class OcrAttempt(result: OcrResult) : UnitWiseEvent(
        name = "ocr_scan_attempt", 
        params = mapOf("result" to result.value)
    ) {
        enum class OcrResult(val value: String) {
            SUCCESS("success"),
            RETRY("retry"),
    ...
    class ParityToggled(isExpanded: Boolean) : UnitWiseEvent(
        name = "asymmetric_parity_viewed",
        params = mapOf("action" to if (isExpanded) "expand" else "collapse")
    )

    class ListParityType(isDual: Boolean) : UnitWiseEvent(
