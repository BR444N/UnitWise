...
class ShoppingListDetailsViewModel(
    ...
    val uiState: StateFlow<ShoppingListDetailsUiState> = combine(
        ...
        items.forEach { item ->
            val priceA = if (item.productAPrice.isNotBlank()) PriceUtils.parsePrice(item.productAPrice) else 0.0
            val priceB = if (item.productBPrice.isNotBlank()) PriceUtils.parsePrice(item.productBPrice) else 0.0
            ...
            if (item.isProductAWinner != null || item.isTie == true) {
                ...
                smart += if (item.isProductAWinner == true || item.isTie == true) {
                    priceA
                } else {
                    priceB
                }
