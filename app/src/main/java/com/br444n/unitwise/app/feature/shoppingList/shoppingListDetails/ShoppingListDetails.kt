...
fun ShoppingListDetailsScreen(
    ...
    viewModel: ShoppingListDetailsViewModel =
        viewModel(factory = ShoppingListDetailsViewModel.Factory),
) {
    ...
    val (firstItemCoordinates, setFirstItemCoordinates) =
        remember {
            mutableStateOf<LayoutCoordinates?>(null)
        }
    val (orphanCardCoordinates, setOrphanCardCoordinates) =
        remember {
            mutableStateOf<LayoutCoordinates?>(null)
        }
...
private fun ShoppingListDetailsList(
    ...
    if (items.isEmpty()) {
        AppEmptyState(
            title =
                if (isSearchActive) {
                    stringResource(id = R.string.details_search_empty_title)
                } else {
                    stringResource(id = R.string.details_empty_title)
                },
            subtitle =
                if (isSearchActive) {
                    stringResource(id = R.string.details_search_empty_subtitle)
                } else {
                    stringResource(id = R.string.details_empty_subtitle)
                },
            iconResId =
                if (isSearchActive) {
                    R.drawable.empty_products
                } else {
                    R.drawable.empty_state_list_details
                },
        )
    } else {
            ...
            Box(
                modifier =
                    if (index ==
                        ...
                        Modifier.onGloballyPositioned { onFirstItemPositioned(it) }
                    } else {
                        Modifier
                    },
            ) {
...
@Composable
private fun ShoppingListDetailsBackButton(
    contentDesc: String,
    onClick: () -> Unit,
) {
    UnitWiseTooltip(
...
@Composable
private fun ShoppingListSelectionCancelButton(
    contentDesc: String,
    onClick: () -> Unit,
) {
    UnitWiseTooltip(
...
private fun ShoppingListShowcaseOverlayHandler(
    ...
    val currentStep =
        remember(uiState, seenFeatures, firstItemCoordinates, orphanCardCoordinates) {
            when {
    ...
    val (config, targetCoordinates, onDismiss) =
        when (currentStep) {
            ShoppingListShowcaseStep.ADD_PRODUCT ->
                Triple(
                    AppShowcaseConfig(
            ...
            ShoppingListShowcaseStep.DELETE_ITEM ->
                Triple(
                    AppShowcaseConfig(
            ...
            ShoppingListShowcaseStep.ORPHAN ->
                Triple(
                    AppShowcaseConfig(
