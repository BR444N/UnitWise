    ...
    Scaffold(
        bottomBar = {
            if (shouldShowBottomBar) {
                UnitWiseBottomNavigation(
                    ...
                    onNavigate = { index ->
                        handleBottomTabNav(index, selectedIndex, navController)
                    },
                )
            }
        },
        ...
        NavHost(
            ...
            composable(
                ...
                val targetProduct =
                    backStackEntry.arguments?.getString("targetProduct") ?: return@composable
                val homeState = homeViewModel.uiState.collectAsState().value
                val inheritedUnit =
                    when (targetProduct) {
                        "A" ->
                            homeState.productB.selectedUnit.takeIf {
                        ...
                        "B" ->
                            homeState.productA.selectedUnit.takeIf {
                                shouldInheritUnitForScan(
...
private fun HomeResultHandler(
    ...
    val inlineComparisonItemId: Int? =
...
private fun handleBottomTabNav(
    index: Int,
    current: Int,
